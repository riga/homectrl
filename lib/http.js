...
/**
 * Helper function to send json formatted responses. Each json object has three attributes:
 * status, data and message. See parameters for explanation. Note that this function does not set
 * http status code of the response object.
 *
 * @example
 * send(res);               // sends "{status: 200, data: null, message: null}"
 * send(res, "foo");        // sends "{status: 200, data: "foo", message: null}"
 * send(res, {a:1});        // sends "{status: 200, data: {a:1}, message: null}"
 * send(res, 400);          // sends "{status: 400, data: null, message: 'Bad Request'}"
 * send(res, 400, "myMsg"); // sends "{status: 400, data: null, message: 'myMsg'}"
 *
 ...
 * @returns {object} The created json object.
 */
