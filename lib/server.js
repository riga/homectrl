...
var Server = Emitter._extend({
    ...
    process.nextTick(function() {
      self.setup(function(err) {
        if (err) {
          throw err;
          // TODO
          return console.error("Error during server setup: " + String(err));
        }
  ...
  setup: function(callback) {
    ...
    // plugin rendering
    var pluginRoot = http.join(this.config.get("app:root"), "plugins");
    this.app.use(pluginRoot, pluginRender(this));

    // static
    var staticRoot = http.join(this.config.get("app:root"), "static");
    ...
    var wsRoot = http.join(this.config.get("app:root"), "socket.io");
    var wsPort = this.config.get("websocket:port");

    this.io = io(wsPort, { path: wsRoot });
