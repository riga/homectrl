...
var Plugin = Emitter._extend({
  ...
  init: function(server, name) {
    ...
    this.dynamicRoot  = server.config.get("app:root") + "plugins/" + name;
    this.staticRoot   = server.config.get("app:root") + "plugins/" + name + "/static";
    this.templateData = {};
    ...
    this.GET("/_template", function(req, res) {
      ...
      var renderData = util._extend({}, defaultData);

      var reqData = req.param("data");
      if (reqData) {
        util._extend(renderData, reqData);
      }

      var tmplData = self.templateData[path];
      if (tmplData) {
        util._extend(renderData, tmplData);
      }
