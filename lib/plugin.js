...
module.exports = Emitter._extend({
  ...
  init: function init(server, name) {
    ...
    fs.readdirSync(path.join(this.root, "conf")).forEach(function(name) {
      var file = path.join(self.root, "conf", name);
    ...
    // less
    server.app.use(this.staticRoot, less(path.join(this.root, "static"), server.lessConfig));

    // static mount point
    var cacheOpts = server.config.get("cache");
    server.app.use(this.staticRoot, express.static(path.join(this.root, "static"), cacheOpts));
