...
module.exports = function(server) {
  ...
  server.config.get("client:availableLanguages").forEach(function(lang) {
    try {
      ...
      server.logger.error("cannot load locales: %s", err);
    }
  });
  ...
  GET("/", "/index", function(req, res) {
    ...
    // get the language
    var lang = server.config.get("client:language");
    if (lang == "auto") {
      lang = req.acceptsLanguages(server.config.get("client:availableLanguages")) || "en";
    }

    // prepare template data
    var _hcData = extend({ lang: lang }, hcData);
    var data = {
      sessionId    : req.session.id,
      locales      : locales[lang],
      localesString: JSON.stringify(locales[lang]),
      hcData       : _hcData,
